...
namespace gyp {
  ...
  void song_map::load(const char * filename) {
  ...
  void song_map::load(std::string filename) {
    ...
    for (pt::ptree::value_type& single_track : tree.get_child("notes")) {
      for (pt::ptree::value_type& single_note : single_track.second.get_child("time")) {
        notes[single_track.second.get<int>("track")].push_back(single_note.second.get_value<int>());
      }
    }
  }
  void song_map::save(const char * filename) {
  ...
  void song_map_db::load(const char * filename) {
    load(std::string(filename));
  }
  void song_map_db::load(std::string filename) {
    ...
    for (pt::ptree::value_type& single_song : tree.get_child("song_maps")) {
      content.push_back(std::make_pair(single_song.second.get_value<std::string>(), song_map()));
      content.back().second.load(content.back().first);
    }
  }
  void song_map_db::save(const char * filename) {
    save(std::string(filename));
  }
